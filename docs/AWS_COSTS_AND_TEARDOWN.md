...
| Amazon Cognito | Admin-invite OAuth code + PKCE, software TOTP MFA | One operator is far below the [10,000 direct/social MAU allowance](https://aws.amazon.com/cognito/pricing/). The template uses software TOTP, not metered SMS MFA. | User pool is intentionally retained and must be deleted manually. |
...
| GuardDuty Malware Protection for S3 | Scans only the quarantine prefix | The [first 1,000 requests and 1 GB scanned per month are free](https://aws.amazon.com/guardduty/pricing/); excess scan bytes/objects and associated S3 operations consume credits. Use only tiny approved test images. Full-account GuardDuty is not enabled by this template. | Malware protection plan and its role delete with the stack. |
