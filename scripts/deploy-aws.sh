...
guardduty_malware_protection_enabled=${GUARDDUTY_MALWARE_PROTECTION_ENABLED:-true}
...
case "$guardduty_malware_protection_enabled" in
  true|false) ;;
  *) echo "GUARDDUTY_MALWARE_PROTECTION_ENABLED must be 'true' or 'false'." >&2; exit 2 ;;
esac
...
if [[ "$sam_build_mode" == "container" ]]; then
  ...
  if [[ "$(uname -s)" != "Linux" || "$(uname -m)" != "x86_64" ]]; then
    echo "SENTINEL_SAM_BUILD_MODE=native-linux requires a Linux x86_64 host." >&2
    exit 2
  fi
...
sam deploy \
  ...
  --parameter-overrides \
    ...
    "GuardDutyMalwareProtectionEnabled=$guardduty_malware_protection_enabled" \
